...
export default function AdminEngAccountCard({ payoutData, userId, onUpdate }: { payoutData: any, userId: string, onUpdate: () => void }) {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok) { toast.success("Payout updated"); onUpdate(); setIsOpen(false); }
      else toast.error("Failed to update payout");
  ...
  return (
    <>
      ...
      <EngineerModal isOpen={isOpen} onClose={() => setIsOpen(false)} className="max-w-[600px]">
        <div className="p-8">
          <h4 className="text-2xl font-bold font-inter text-[var(--text-primary)] mb-6">Edit Payout Configuration</h4>
